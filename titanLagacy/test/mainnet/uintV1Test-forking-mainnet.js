...
describe("V1Test - forking mainnet", function () {
    const erc20ABI = [
        {
            inputs: [
    ...
    describe("Deploy Contract", () => {
        ...
        it("Deploy UpgradeL1Bridge", async () => {
            const UpgradeL1BridgeDep = new ethers.ContractFactory(
                ...
                UpgradeL1BridgeV1_ABI.bytecode,
                tester
            )
    ...
    describe("setting the Contract", () => {
        ...
        it("SetCode", async () =>{
            await StandardBridgeProxy.connect(BridgeOwner).setCode(
                UpgradeL1BridgeV1_ABI.deployedBytecode
            );
        })

        it("Set UpgradeL1BridgeLogicV1 Contract", async () => {
                    ...
                    for(let k = 0; k < 1; k++) {
                        ...
                        if(Account.toUpperCase() != testZeroAddr.toUpperCase()){
                            ...
                            if (code !== '0x') {
                                ...
                                console.log("before getClaimHash : ", getClaimHash)
                                ...
                                getClaimHash = await UpgradeL1BridgeLogic.connect(tester).claimState(Hash)
                                console.log("after getClaimHash : ", getClaimHash)
